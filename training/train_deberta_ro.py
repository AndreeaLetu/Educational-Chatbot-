...
SAVE_NAME  = "tinylogic_modernbert"
...
class MultiTaskDeBERTaRO(nn.Module):
...
def main():
    print("\n" + "="*60)
    print(f"  TinyLogic — Fine-Tuning DeBERTa Românesc")
    print(f"  Model: {MODEL_NAME}")
    print(f"  Task:  Multi-task (Label + Difficulty)")
    print(f"  Save:  {SAVE_NAME}")
    print(f"  Îmbunătățiri: Mean Pooling + Label Smoothing + Cosine Scheduler")
    print("="*60 + "\n")
    ...
    print("  (prima rulare descarcă modelul — așteptați...)")
    ...
    model = MultiTaskDeBERTaRO(MODEL_NAME, num_labels, num_difficulties, FREEZE_LAYERS)
    ...
    print(f"  Loss Diff Weight: {LOSS_DIFF_WEIGHT}")

    print(f"\nStartăm antrenamentul ({EPOCHS} epoci)...")
    print(f"  Estimare timp CPU: ~{len(train_loader) * EPOCHS * 4 // 60} minute\n")
  ...
  ══════════════════════════════════════════
    ...
    config = {
        ...
        "loss_diff_weight":  LOSS_DIFF_WEIGHT,
        "pooling":           "mean",
