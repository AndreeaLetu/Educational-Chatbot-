...
MODEL_DIR = os.path.join(BASE_DIR, "models", "final", "tinylogic_xlmroberta")
DATA_DIR  = os.path.join(BASE_DIR, "data", "augmented")
OUT_DIR   = os.path.join(BASE_DIR, "diagrams", "xlmroberta_v1")
...
def main():
    ...
    model = MultiTaskModel(MODEL_NAME, NUM_LABELS, NUM_DIFFICULTIES, FREEZE_LAYERS)
    ...
    plot_cm(confusion_matrix(yt_l,yp_l), LABEL_NAMES, f"XLM-RoBERTa varianta 1 — Label\nAcc={la:.2%}  F1={lf:.2%}", os.path.join(OUT_DIR,"cm_label"))
    plot_cm(confusion_matrix(yt_d,yp_d), DIFF_NAMES,  f"XLM-RoBERTa varianta 1  — Dificultate\nAcc={da:.2%}  F1={df:.2%}", os.path.join(OUT_DIR,"cm_diff"))
