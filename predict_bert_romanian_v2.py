...
MODEL_DIR = os.path.join(BASE_DIR, "models", "final", "tinylogic_bert_romanian_v2")
DATA_DIR  = os.path.join(BASE_DIR, "data", "augmented")
OUT_DIR   = os.path.join(BASE_DIR, "diagrams", "bert_romanian_v2")
...
def main():
    ...
    plot_cm(confusion_matrix(yt_l,yp_l), LABEL_NAMES, f"BERT Românesc varianta 2 — Label\nAcc={la:.2%}  F1={lf:.2%}", os.path.join(OUT_DIR,"cm_label"))
    plot_cm(confusion_matrix(yt_d,yp_d), DIFF_NAMES,  f"BERT Românesc varianta 2 — Dificultate\nAcc={da:.2%}  F1={df:.2%}", os.path.join(OUT_DIR,"cm_diff"))
