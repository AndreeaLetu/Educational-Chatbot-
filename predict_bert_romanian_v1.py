...
MODEL_DIR = os.path.join(BASE_DIR, "models", "final", "tinylogic_bert_romanian")
DATA_DIR  = os.path.join(BASE_DIR, "data", "augmented")
OUT_DIR   = os.path.join(BASE_DIR, "diagrams", "bert_romanian_v1")
...
class MultiTaskBERT(nn.Module):
    def __init__(self, model_name, num_labels, num_difficulties, freeze_layers=4):
        ...
        self.difficulty_head = nn.Sequential(
            nn.Dropout(0.3), nn.Linear(h, 128), nn.ReLU(),
            nn.Dropout(0.1), nn.Linear(128, num_difficulties)
        )

    def forward(self, input_ids, attention_mask):
        out = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        cls = out.last_hidden_state[:, 0, :]
        return self.label_head(cls), self.difficulty_head(cls)
...
def run_inference(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            yp_l.extend(torch.argmax(ll, 1).cpu().tolist())
            yt_l.extend(batch["label"].tolist())
            yp_d.extend(torch.argmax(dl, 1).cpu().tolist())
...
def main():
    print("\n" + "=" * 50)
    print("BERT Românesc varianta 1")
    print("=" * 50)
    ...
    loader = DataLoader(
        TinyLogicDataset(test_df, tokenizer, MAX_LENGTH),
        batch_size=BATCH_SIZE, shuffle=False
    )

    model = MultiTaskBERT(MODEL_NAME, NUM_LABELS, NUM_DIFFICULTIES, FREEZE_LAYERS)
    model.load_state_dict(torch.load(
        os.path.join(MODEL_DIR, "model_weights.pt"), map_location=device
    ))
    model.to(device)
    ...
    print(f"  Label — Acc: {label_acc:.4f} | F1: {label_f1:.4f}")
    ...
    print("  Generez heatmap-uri confusion matrix...")

    plot_cm(
        confusion_matrix(yt_l, yp_l), LABEL_NAMES,
        f"BERT Românesc varianta 1 — Label\nAcc={label_acc:.2%}  F1={label_f1:.2%}",
        os.path.join(OUT_DIR, "cm_label")
    )
    plot_cm(
        confusion_matrix(yt_d, yp_d), DIFF_NAMES,
        f"BERT Românesc varianta 1 — Dificultate\nAcc={diff_acc:.2%}  F1={diff_f1:.2%}",
